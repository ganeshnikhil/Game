...
class enenmy:
   def __init__(self,board_positions):
      ...
      self.direction_change_interval = random.randint(100,300)  # Change direction every 100 milliseconds
      
      
   def move_ghost(self, ghost_x, ghost_y,delay):
        ...
        if delay%50==0:
            ...
            if direction == 'up' and ghost_x - 1 >= 0:
                    ghost_x -= 1
            elif direction == 'down' and ghost_x + 1 < len(self.board_positions):
                    ghost_x += 1
            elif direction == 'left' and ghost_y - 1 >= 0:
                    ghost_y -= 1
            elif direction == 'right' and ghost_y + 1 < len(self.board_positions[0]):
                    ghost_y += 1
...
while True:
    ...
    for i,location in enumerate(ghost_location):
        ...
        if ghost.collison_detect(ghost_x,ghost_y,player_x,player_y):
            ...
            ghost_x,ghost_y = ghost.move_ghost(ghost_x,ghost_y,delay)
            ghost_location[i][0],ghost_location[i][1]=ghost_x,ghost_y
            ghost.enemy_draw(ghost_x,ghost_y)
    #new_ghost_location = []
    #for ghost_x, ghost_y in ghost_location:
        #ghost_x, ghost_y = ghost.move_ghost(ghost_x, ghost_y)
        #new_ghost_location.append([ghost_x, ghost_y])
   
    #ghost_location = new_ghost_location
    #for ghost_x, ghost_y in ghost_location:
        #ghost.enemy_draw(ghost_x, ghost_y)
    # Update the screen
    delay+=1
